...
# Upload file
uploaded_file = st.file_uploader("Upload File", type=["zip", "tar.gz"])
...
if uploaded_file is not None:
    ...
    if st.button("Deploy"):
        ...
        if upload_success:
            ...
            if extract_error:
                st.error(f"Gagal mengekstrak file: {extract_error}")
            else:
                # Restart web server
                ...
                if restart_error:
                    st.error(f"Gagal restart web server: {restart_error}")
                else:
                    st.success("Deploy berhasil!")
        else:
            st.error(f"Gagal mengunggah file: {upload_error}")
...
if st.button("Hapus File"):
    ...
    st.write("File berhasil dihapus.")